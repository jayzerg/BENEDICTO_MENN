...
export default function TeacherLayout({ children }) {
  ...
  const [showAddStudent, setShowAddStudent] = useState(false); // New state for Add Student modal
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (userData.profilePicture) {
            ...
            localStorage.setItem('teacherProfileImage', userData.profilePicture);
          } else {
  ...
  const generateAvatar = (name, facultyId) => {
    const initials = getInitials(name);
    const bgColor = stringToColor(facultyId || name || 'teacher');
            ...
            <div className="relative">
              ...
              {showProfileMenu && (
                <div className={`absolute right-0 mt-2 w-48 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg py-2`}>
                  ...
                  {/* Add Student button placed between Settings and Change Password */}
                  <button
                    onClick={() => { setShowAddStudent(true); setShowProfileMenu(false); }}
                    className={`block w-full text-left px-4 py-2 ${darkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100'}`}
                  >
                    Add Student
                  </button>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white'} rounded-lg p-6 w-96 max-w-md`}>
            <h2 className="text-xl font-bold mb-4">Teacher Settings</h2>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white'} rounded-lg p-6 w-96 max-w-md`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setStudentForm({...studentForm, lastName: e.target.value.replace(/[0-9]/g, '')})}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setStudentForm({...studentForm, firstName: e.target.value.replace(/[0-9]/g, '')})}
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="123456"
                    className={`w-full px-3 py-2 pl-16 border rounded-lg ${studentIdError ? 'border-red-500 animate-pulse bg-red-50' : darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'}`}
                    value={studentForm.studentId.replace('2025-', '')}
                    onChange={(e) => {
                      const value = e.target.value.replace(/\D/g, '').slice(0, 6);
                      setStudentForm({...studentForm, studentId: `2025-${value}`});
                      setStudentIdError('');
                    }}
                    maxLength="6"
                  />
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <input
                    ...
                    onDoubleClick={() => {
                      navigator.clipboard.writeText(studentForm.password);
                      // Optional: Show a visual feedback that the password was copied
                      const originalValue = studentForm.password;
                      setStudentForm({...studentForm, password: 'Copied!'});
                      setTimeout(() => {
                        setStudentForm({...studentForm, password: originalValue});
                      }, 1000);
                    }}
            ...
            <div className="flex space-x-3 mt-6">
              <button
                onClick={async () => {
                  const studentIdNumber = studentForm.studentId.replace('2025-', '');
                  if (studentIdNumber.length !== 6) {
                    setStudentIdError('Student ID must be exactly 2025- followed by 6 digits');
                    return;
                  }
                  ...
                  try {
                    const response = await fetch('/api/auth/register-student', {
                      ...
                      body: JSON.stringify({
                        ...studentForm,
                        facultyId: studentForm.studentId, // Use studentId as facultyId for students
                        role: 'student'
                      }),
                    });
                    ...
                    if (response.ok) {
                      ...
                      setStudentIdError(data.message || 'Student with this ID already exists');
                    }
                  } catch (error) {
